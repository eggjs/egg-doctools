...
class BaseCommand extends Command {
  constructor(rawArgv) {
    ...
    this.options = {
      ...
      deployBranch: {
        type: 'string',
        default: 'gh-pages',
        description: 'branch to deploy doc',
      },
      publisherName: {
        type: 'string',
        default: 'Auto Doc Publisher',
        description: 'publisher name',
      },
      publisherMail: {
        type: 'string',
        default: 'docs@eggjs.org',
  ...
  async prepare({ docPath, targetDir, external }) {
    ...
    // $baseDir/docs/.vuepress/public > $targetDir/.vuepress/public/*
    // $baseDir/docs/.assets > $targetDir/.vuepress/public/*
    try {
      await this.copyDir(
        path.join(docPath, '.vuepress/public'),
        path.join(targetDir, '.vuepress/public')
      );

      await this.copyDir(
        path.join(docPath, '.assets'),
        path.join(targetDir, '.vuepress/public')
      );
    } catch (error) {
      this.logger.info('Copy Public Resources: Skip');
    }
